...
### Requirements:
This branch requires a stream being broadcast over the network by [`ffmpeg-ws-relay`](https://github.com/geoffreycs/ffmpeg-ws-relay-modernized). The default IP address is set to something you probably will not want to use, so you can either edit `window.js` or type in a new value at runtime.
...
WebAssembly backend: `npm run wasm`  
JavaScript backend: `npm run js`  
...
Presently, it does not perform very well on the Raspberry Pi 5, as WebGPU support on Linux is virtually broken, limiting us to the less-efficient WebGL backend.